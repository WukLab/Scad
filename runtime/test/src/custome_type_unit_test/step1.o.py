...
def main(params, action):
    dt = np.dtype([(str(i), 'f4') for i in range(20)])
    array_slice_1 = np.zeros(200, dtype=np.float32)
    array_slice_1 = array_slice_1.view(dtype=dt)
    array_slice_2 = np.ones(200, dtype=np.float32)
    array_slice_2 = array_slice_2.view(dtype=dt)
    array_slice_3 = np.zeros(200, dtype=np.float32)
    array_slice_3 = array_slice_3.view(dtype=dt)
